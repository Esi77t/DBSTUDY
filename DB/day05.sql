...
-- 단, 각 행의 모든 가능한 조합을 만들기 때문에 결과가 매우 큰 테이블 될 수 있으므로 주의 필요
...
-- LEFT OUTER JOIN
-- '왼쪽 테이블의 모든 행'과 '오른쪽 테이블과 왼쪽 테이블이 공통적으로 가지는 값'을 반환
...
SELECT * FROM STADIUM s RIGHT OUTER JOIN TEAM t
...
-- LEFT와 RIGHT 중 상황에 따라 다름
-- 대부분 왼쪽 테이블의 데이터를 중심으로 분석하고자 하는 경우가
-- 많기 때문에 LEFT를 더 많이 사용

-- FULL OUTER JOIN
-- 두 테이블에서 '모든 값'을 반환
-- 서로 공통되지 않은 부분은 NULL로 채움
-- 합집합의 연산과 같음
SELECT FIRST_NAME, DEPARTMENT_NAME
...
SELECT * FROM EMP;
SELECT * FROM DEPT;
-- 부서번호, 사원명, 직업, 위치를 EMP와 DEPT테이블을 통해 INNER JOIN하여 조회
SELECT d.DEPTNO, ENAME, JOB, LOC
FROM EMP e INNER JOIN DEPT d
ON e.DEPTNO = d.DEPTNO;

-- PLAYER 테이블, TEAM 테이블에서 송종국 선수가 속한 팀의 전화번호 조회하기
-- 팀 아이디, 선수 이름, 전화번호 조회
...
SELECT e.JOB_ID, j.JOB_TITLE, e.EMAIL, FIRST_NAME || ' ' || LAST_NAME "이름" 
FROM JOBS j JOIN EMPLOYEES e 
ON e.JOB_ID = j.JOB_ID;

-- 비등가조인
-- 두 테이블을 결합할 때 부등호(>, <, >=, >=), BETWEEN, LIKE 등
-- 다양한 비교 연산자를 통해 조인조건을 지정하는 방식
...
-- DEPT 테이블의 LOC 별 평균 SAL를 반올림 한 값과 SAL의 총합을 조회
SELECT LOC, ROUND(AVG(SAL)), SUM(SAL)
...
-- 자동매칭
-- 두 테이블에서 이름이 동일한 컬럼을 찾아서, 해당 컬럼들이 값이 일치하는 행끼리 자동으로 결합
-- 중복 컬럼제거
-- 조인 결과에서 공통 컬럼은 한번만 표시
-- 명시적 조건 생략
-- ON절이나 USING절 없이 간단하게 조인 가능

-- 의도하지 않는 결과가 나올 수 있음
-- 자동으로 공통컬럼을 기준으로 조인하기 때문에, 개발자가 어떤 컬럼을
-- 기준으로 조인하는지 명확히 알기 어려움
...
-- VIEW
-- 하나 이상의 테이블이나 다른 뷰의 데이터를 볼 수 있게 해주는 데이터베이스 객체
-- 쿼리 결과를 마치 하나의 테이블처럼 사용할 수 있도록 하는 가상의 테이블
-- 실제 데이터는 저장하지 않고, 뷰를 참조할 때마다 미리 정의된 쿼리 문이 실행되어 결과가 생성
...
-- 편리성
-- 복잡한 쿼리문을 뷰로 생성함으로써 관련 쿼리문을 단순하게 작성할 수 있음
-- 자주 사용하는 SQL문이면 뷰에 저장하고 편리하게 사용할 수 있음
-- 보안성
-- 숨기고 싶은 정보가 존재한다면, 뷰를 생성할 때 해당 컬럼은 빼고
-- 생성함으로써 사용자에게 정보를 감출 수 있음
...
-- CASCADE : 뷰를 참조하는 다른 뷰나 제약 조건까지 모두 삭제 (함부로 쓰지 말 것)
...
-- King Steven의 부하직원이 몇 명인지 조회
SELECT COUNT(*) "King Steven의 부하 직원"
FROM EMPLOYEES_MANAGER

WHERE MNAME = 'King Steven';

-- PLAYER테이블에 TEAM_NAME 컬럼을 추가한 VIEW 만들기
-- VIEW이름은 PLAYER_TEAM_NAME
...
CREATE OR REPLACE VIEW STADIUM_INFO AS(
	SELECT HOMETEAM_ID, s.STADIUM_NAME, TEAM_NAME
...
-- SELECT, INSERT, UPDATE, DELETE문을 하나의 작업 단위라고 함

-- CASE문
-- 데이터 값을 WHEN의 조건과 차례대로 비교한 후 일치하는 값을 찾아
-- THEN 뒤에 있는 결과 값을 반환
SELECT
	ENAME,
	DEPTNO,
...
-- EMPLOYEES의 JOB_ID에서 IT_PROG의 평균 급여를 소수점 2자리까지 반올림해서 구해라
SELECT ROUND(AVG(CASE JOB_ID WHEN 'IT_PROG' THEN SALARY END), 2) "평균 급여"
FROM EMPLOYEES;
...
-- 전부 아니면 LOW를 ENAME, SAL, GRADE순으로 조회
SELECT
	ENAME,
	SAL,
	CASE 
...
-- NULL 처리
-- 조건에 해당하지 않는 경우, ELSE절이 없으면 NULL로 반환
-- 중첩 사용가능
-- CASE문 안에 CASE문을 중첩해서 사용 가능
-- 표준 SQL 지원
-- 대부분의 주요 데이터베이스 등에서 표준 SQL문법의 일부로 지원 됨.


-- PL(Procedural Language) / SQL문
-- 원래 SQL문은 주로 데이터의 정의, 조작, 제어를 위한 언어
-- PL / SQL은 여기에 조건, 반복문, 변수 선언, 예외 처리와 같은
-- 절차적 기능을 추가하여 복잡한 로직을 구현할 수 있게 해줌

-- PL / SQL문의 구조
-- 기본적으로 블록 단위로 구성. 하나의 블록은 세 부분으로 이루어져 있음
...
BEGIN
	...
	DBMS_OUTPUT.PUT_LINE(v_message);	-- console.log()와 비슷한 것
END;
...
BEGIN
	...
	ELSE GRADE := 'F';
	END IF;
	DBMS_OUTPUT.PUT_LINE('당신의 점수는 ' || SCORE ||'점' || CHR(10) /* 줄바꿈 */ || '학점은 ' || GRADE);
END;
...
-- 1. 익명 블록
-- 이름 없이 한 번 실행되는 PL / SQL 블록
-- 데이터베이스에 저장되지 않고 즉시 실행
