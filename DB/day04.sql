...
-- 자동으로 만들어진 INDEX이든, 우리가 수동으로 만든 INDEX이든 조회할 때 사용이 되는 거라면
-- 잘 사용이 되고 있는지 확인을 해야할 필요가 있다
-- EXPLAIN PLAN FOR
-- ORACLE 데이터 베이스에서 SQL 쿼리가 실행될 때 어떤 경로로 수행되는지를 미리 확인할 수 있는 명령어
-- 주요 개념
-- 실행 계획(EXPLAIN PLAN)
-- SQL 쿼리를 처리하기 위한 단계별 작업순서를 나타냄
...
-- 이후 DBMS_XPLAN.DISPLAY함수를 이용하여 보기 쉽게 출력 가능
EXPLAIN PLAN FOR
...
SELECT * FROM (SELECT *
			   FROM PLAYER
			   WHERE TEAM_ID = 'K01')  -- 테이블처럼 쓴다는 의미
WHERE "POSITION" = 'GK';
...
SELECT (SELECT (ROUND(AVG(HEIGHT), 1)) FROM PLAYER)"전체 평균 키", "POSITION", (ROUND(AVG(HEIGHT), 1))"포지션 별 평균 키"
FROM PLAYER
...
-- EMPLOYEES테이블에서 평균 급여보다 급여가 낮은 사원들의 급여를 10% 인상하기
SELECT AVG(SALARY)
FROM EMPLOYEES;
...
-- 어떤 테이블에서 온 컬럼인지 확실하게 해야될 때가 있다
...
-- 부서테이블과 지역(LOCATIONS)로부터 부서명과 CITY조회
...
-- 사원, 부서, 지역 테이블로부터 이름, 이메일, 부서번호, 부서명, 지역번호, 도시명을 조회하되, 도시가 'Seattle'인 경우만 조회
